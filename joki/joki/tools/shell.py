...
_FILE_EDIT_PATTERNS = re.compile(
    r'(^|[|;])\s*'
    r'(sed\s+-i|echo\s+.*[>»]|awk\s+.*[>»]|printf\s+.*[>»]'
    r'|cat\s+.*[>»]|tee\s+)',
    re.IGNORECASE
)
...
def handle_run_command(args):
    ...
    if is_interactive:
        output = _run_with_pty(cmd, timeout=timeout_s, idle_timeout=idle_timeout_s)
    else:
        output = _shell_execute(cmd, timeout=timeout_s, idle_timeout=idle_timeout_s)
