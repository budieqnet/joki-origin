...
def test_log_rewraps_on_resize(tui_queue):
    ...
    async def run():
        ...
        async with app.run_test(size=(120, 30)) as pilot:
            ...
            assert len(msg_narrow.visual.plain) > 100, "isi pesan tidak boleh terpotong"
...
def _menu_state(text):
    ...
    return JokiApp._cmd_menu_state(fake)
...
def test_menu_model_suboptions_on_exact_command():
    phase, options = _menu_state("/model")
    assert phase == "model"
...
def test_menu_model_suboptions_filtered():
    phase, options = _menu_state("/model de")
    assert phase == "model"
    assert options
    for k, n in options:
...
def test_menu_hides_after_exact_model_selected():
    ...
    assert _menu_state("/model deepseek-v4-flash") is None


def test_menu_lsp_suboptions_on_exact_command():
    phase, options = _menu_state("/install-lsp")
    assert phase == "lsp"
    keys = [k for k, _ in options]
...
def test_menu_lsp_suboptions_filtered():
    phase, options = _menu_state("/install-lsp py")
    assert phase == "lsp"
...
def test_compat_legacy_alt_enter_parses_to_alt_enter():
    ...
    parser = XTermParser(lambda *a, **k: None)
